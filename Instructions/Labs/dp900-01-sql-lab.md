...
3. Enter the following values on the  **Create SQL Database**  page:
    ...
    -   **Resource group**: If you're using a  _sandbox_  subscription, select the existing resource group (which will have a name like  _learn-xxxx..._). Otherwise, select existing resource group with a name  like <inject key="Deployment-id" enableCopy="false" />.
